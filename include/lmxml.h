...
/** helper */
static inline xmlDocPtr xml_open(const std::string& name, const std::string& encoding) {
    xmlDocPtr doc;
...
static inline std::string get_xml_attr(xmlDocPtr doc, const std::string& name, const std::string& prop) {
    xmlNodePtr node;
    xmlAttrPtr attr;
    xmlChar* xvalue;
    std::string svalue;
...
template<class T>
static inline void get_xml_integer(xmlDocPtr doc, const std::string& name, T* t) {
    std::string sval;
    sval = get_xml_val(doc, name);
    *t = atoll(sval.c_str());
}

static inline void get_xml_float(xmlDocPtr doc, const std::string& name, double* t){
    std::string sval;
    sval = get_xml_val(doc, name);
    *t = atof(sval.c_str());
}

static inline void get_xml_array(xmlDocPtr doc, const std::string& name, std::vector<std::string> *vec) {
    size_t pos;
    xmlNodePtr root;
    xmlNodePtr node;
...
template <class T>
static inline void get_xml_array_int(xmlDocPtr doc, const std::string& name, std::vector<T> *vec) {
    int64_t intval;
...
template <class T>
static inline void get_xml_array_float(xmlDocPtr doc, const std::string& name, std::vector<T> *vec) {
    double dblval;
    std::vector<std::string> strlist;
    get_xml_array(doc, name, &strlist);
