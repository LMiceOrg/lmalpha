...
#define eal_timer_free_context(ctx) free(ctx);
...
#if EAL_STDC_VER == 90
#define eal_time_factor(factor) do{\
...
extern void* eal_timer_thread(void*);
...
forceinline int eal_timer_create_dispatch(dispatch_source_t* timer,
                                          const char* name, uint64_t* interval,
                                          uint64_t* leeway,
