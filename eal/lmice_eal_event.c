...
forceinline void hash_to_nameA(uint64_t hval, char* name) {
  int i = 0;
...
int eal_event_create(lmice_event_t* e) {
  e->fd = CreateEventA(NULL, FALSE, FALSE, e->name);
  if (e->fd == NULL) {
    DWORD hr = GetLastError();
...
int eal_event_wait_one(evtfd_t fd) {
  DWORD hr = WaitForSingleObject(fd, INFINITE);
  return hr;
}
...
int eal_event_create(lmice_event_t* e) {
  int ret = 0;
  e->fd = sem_open(e->name, O_CREAT | O_EXCL, 0666, 0);
  if (e->fd == SEM_FAILED) {
    ret = errno;
    sem_unlink(e->name);
    e->fd = 0;
...
int eal_event_wait_one(evtfd_t fd) { return sem_wait(fd); }
