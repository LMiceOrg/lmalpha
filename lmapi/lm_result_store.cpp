...
int factor_result::store_factor(const lmapi_result_info& info) {
  ...
  try {
    ...
    if (exists >= 1) {
      ...
      sql->os.open(1,
                   "update alpha.dbo.factor_info set author=:f1<char[32],in>, "
                   "last_update=:f2<char[32],in> where name =:f3<char[32],in>",
                   sql->db);
